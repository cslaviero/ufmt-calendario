<?php
    ...
    $sql = "SELECT prd_id, prd_nome, COUNT(eve_id) as eventos, 
            date_format(prd_data_ini, '%d/%m/%Y %H:%i:%s') as inicio, date_format(prd_data_fim, '%d/%m/%Y %H:%i:%s') as fim
            FROM tbl_periodos, tbl_eventos WHERE eve_periodo = prd_id 
            GROUP BY eve_periodo ORDER BY prd_data_fim DESC;";
    $exe = $Conn->query($sql);
    $num = $exe->num_rows;
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Calendário UFMT - Painel de Controle</title>
                    ...
                    <div class="panel-body">
                        <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            <tr class="<?php if($i % 2 == 0){ echo "odd"; } else { echo "even"; }?> gradeX">
                                ...
                                <td><?=$reg['fim']?></td>
                                <td class="center">
                                    <a role="button" data-toggle="modal" data-target="#ins_evento" class="btn btn-primary btn-xs" onclick="setor(<?=$reg['prd_id']?>);">Inserir evento</a>
                                    <a role="button" class="btn btn-warning btn-xs" href="notification.php">Enviar notificações</a>
                                </td>
                            </tr>
                            <?php $i++; } ?>
                            </tbody>
